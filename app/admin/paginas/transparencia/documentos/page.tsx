...
export default function TransparenciaDocumentosAdmin() {
  ...
  async function uploadArquivo(file: File, gi: number, di: number) {
    ...
    const cp = [...grupos];
    cp[gi].links[di].url = data.publicUrl;
    setGrupos(cp);
  }
  ...
  return (
    <div className="admin-card">
      <h1>Transparência — Documentos Institucionais</h1>
      <p>Gerencie os documentos publicados na página pública.</p>
          ...
          {grupo.links.map((doc, di) => (
            <div key={di} style={{ marginTop: 12 }}>
              ...
              <input
                ...
                onChange={(e) => {
                  const cp = [...grupos];
                  cp[gi].links[di].label = e.target.value;
                  setGrupos(cp);
                }}
              />
