...
**Atualização 2026-08-11 (mesmo dia):** a correção definitiva de `convites_org`
foi implementada, com autorização explícita do usuário e acesso de push
concedido ao repositório `site-ipecc`:

1. Criada `validar_convite_token(p_token text)`, função `SECURITY DEFINER`
   (mesmo padrão de `validar_portal_token`), retornando só os campos públicos
   do convite cujo token bate exatamente — sem enumeração possível. Testada
   no Supabase.
2. `site-ipecc/app/convite/[token]/page.tsx` trocado para chamar essa RPC em
   vez de `.from("convites_org").select(...).eq("token", token)` diretamente
   — branch `fix/convites-org-token-rpc-2026-08-11`, `typecheck` limpo, não
   mergeado em `main` (aguardando revisão).
3. A policy `convites_org_select_temp_aberto` foi removida — `convites_org`
   só tem `service_role_all_convites` agora, sem nenhuma brecha aberta.
