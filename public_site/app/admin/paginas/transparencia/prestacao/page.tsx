...
export default function TransparenciaPrestacaoAdmin() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from("paginas")
        .select("prestacao_contas")
        .eq("slug", "transparencia")
        .single();

      if (data?.prestacao_contas) {
        setGrupos(data.prestacao_contas);
      }
  ...
  async function uploadArquivo(file: File, gi: number, di: number) {
    const path = `transparencia/prestacao/${Date.now()}-${file.name}`;
  ...
  return (
    <div className="admin-card">
      <h1>Transparência — Prestação de Contas</h1>
      <p>Gerencie os relatórios técnicos e financeiros publicados.</p>
