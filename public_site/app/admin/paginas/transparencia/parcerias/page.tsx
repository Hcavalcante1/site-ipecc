...
export default function TransparenciaParceriasAdmin() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (data?.parcerias) {
        setTexto(data.parcerias.texto ?? "");
        setGrupos(data.parcerias.grupos ?? []);
      }
  ...
  async function uploadArquivo(file: File, gi: number, di: number) {
    const path = `transparencia/parcerias/${Date.now()}-${file.name}`;
  ...
  return (
    <div className="admin-card">
      <h1>Transparência — Parcerias</h1>
      <p>Gerencie os instrumentos de parceria firmados.</p>
      ...
      <button
        type="button"
        style={btnGreen}
        onClick={() => setGrupos([...grupos, { titulo: "", itens: [] }])}
      >
        + Adicionar grupo
      </button>
